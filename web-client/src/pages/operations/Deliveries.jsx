...
import { deliveriesAPI, productsAPI, locationsAPI } from '../../api'
import {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-[#0f1117] border border-[#1a1d24] rounded-2xl w-full max-w-lg mx-4 shadow-2xl animate-fade-in">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#1a1d24]">
          <div>
            <h2 className="text-sm font-semibold text-white">New Delivery</h2>
            <p className="text-xs text-gray-600 mt-0.5">Outgoing stock movement</p>
          </div>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input
              type="number" min={1} className="input w-full" placeholder="e.g. 50"
              value={form.quantity} onChange={(e) => set('quantity', e.target.value)}
            />
          </div>

          <div>
            <label className="block text-xs text-gray-500 mb-1.5">Source Location</label>
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-1.5">Customer / Contact</label>
            <input
              type="text" className="input w-full" placeholder="e.g. Reliance Industries"
...
function DeliveryDetail({ delivery, onBack, onRefresh }) {
  ...
  const canAct = delivery.status === 'draft'
          ...
          <div className="card p-5">
            ...
            <div className="grid grid-cols-2 gap-x-6">
              {[
                ...
                { label: 'From Location',      value: delivery.from_location, icon: null     },
                { label: 'To Location',        value: delivery.to_location,   icon: null     },
            ...
            {delivery.remarks && (
              <div className="mt-4 p-3 bg-[#1a1d24] rounded-lg">
                ...
                <p className="text-sm text-gray-300">{delivery.remarks}</p>
              </div>
            )}
          </div>
...
export default function Deliveries() {
  ...
  const openDetail = async (id) => {
    try { const { data } = await deliveriesAPI.getOne(id); setSelected(data) }
    catch { alert('Could not load delivery') }
  }

  const refreshDetail = async () => {
    if (!selected) return
    try { const { data } = await deliveriesAPI.getOne(selected.id); setSelected(data); fetchDeliveries(page) }
    catch {}
  }

  if (selected) return (
    <DeliveryDetail delivery={selected} onBack={() => { setSelected(null); fetchDeliveries(page) }} onRefresh={refreshDetail} />
  )

  return (
    <div className="page-container animate-fade-in">
      {showModal && <NewDeliveryModal onClose={() => setShowModal(false)} onCreated={() => fetchDeliveries(1)} />}

      <div className="page-header flex-wrap gap-4">
        <div>
          <h1 className="page-title">Deliveries</h1>
          <p className="page-subtitle">Outgoing stock operations · {count} total</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => fetchDeliveries(page)} className="btn-secondary btn-sm">
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        ...
        <select className="input w-36" value={statusFilter}
          onChange={(e) => { setStatus(e.target.value); fetchDeliveries(1) }}>
          <option value="">All Status</option>
          <option value="DRAFT">Draft</option>
          <option value="DONE">Done</option>
          <option value="CANCELLED">Cancelled</option>
        </select>
        <button onClick={() => fetchDeliveries(1)} className="btn-secondary btn-sm">
      ...
      <div className="card overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={6}><div className="flex items-center justify-center py-16 gap-2 text-gray-600 text-sm"><RefreshCw size={14} className="animate-spin" /> Loading deliveries...</div></td></tr>
            ) : deliveries.length === 0 ? (
              <tr><td colSpan={6}><div className="text-center py-16"><Truck size={32} className="text-gray-800 mx-auto mb-3" /><p className="text-sm text-gray-600">No deliveries found</p><p className="text-xs text-gray-700 mt-1">Click "New Delivery" to create one</p></div></td></tr>
            ) : deliveries.map((d) => (
              <tr key={d.id} className="table-row" onClick={() => openDetail(d.id)}>
                ...
                <td className="table-cell text-gray-300">{d.contact}</td>
                <td className="table-cell text-gray-300">{d.product_name}</td>
                <td className="table-cell mono text-gray-400">{d.quantity}</td>
                <td className="table-cell text-gray-500">{d.schedule_date}</td>
